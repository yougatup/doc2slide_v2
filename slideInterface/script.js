	...
	function setStatusLight(color) {
		...
		$(document).on("extension_pageUpdated", function(e) {
			...
			if (adaptivePlaneStatus != 2) {
				...
				if(currentResourceSelectedFlag) {
					var slideID = docSlideStructure[currentSelectedSlideIndex].slide.id;

					if(pageID != slideID) {
						hideAlternativeView();
					}
					else {
						var label = docSlideStructure[currentSelectedSlideIndex].contents.list[currentSelectedResourceIndex].currentContent.label;
						var objID = docSlideStructure[currentSelectedSlideIndex].layout.mapping[label];

						var contentsItem = $(".adaptationTableResourceBody[slideindex='" + currentSelectedSlideIndex + "'][resourceindex='" + currentSelectedResourceIndex + "']");

						$(contentsItem).addClass("selectedForAlternative");

						var pIndex = getParagraphIndexOfDocSlideStructure(currentSelectedSlideIndex, currentSelectedResourceIndex);

						console.log(objID);

						var objIndicatorObj = $(".objectIndicator[objid='" + objID + "'][paragraphindex='" + pIndex + "']");

						console.log(objIndicatorObj);

						$(objIndicatorObj).addClass("selectedForAlternative")
					}
				}
